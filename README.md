...
## 附近特色商家／周邊探索（OpenStreetMap，經自己的後端代理）

每個有座標的行程站點，展開後都會顯示「附近特色商家」，「周邊探索」分頁也會依景點／商場／
百貨／藥妝四大類顯示最近的地點——這兩個功能查詢的是 [OpenStreetMap](https://www.openstreetmap.org/)
的 Overpass API 真實店家資料。

**這裡有個架構上的重點**：Overpass 的公開伺服器不支援瀏覽器直接跨網域呼叫（沒有回傳 CORS
標頭），所以跟天氣、匯率那兩個「純前端直接呼叫」的做法不一樣，這個功能改由 `api/nearby.js`
這支後端函式代為查詢——前端呼叫自己網域底下的 `/api/nearby`，再由這支後端去問 Overpass，
伺服器對伺服器的請求不受瀏覽器的 CORS 限制，所以能穩定運作。這支函式**不需要**任何 API
金鑰（Overpass 本身免費、免申請），但因為多了一支後端函式，**一樣沒辦法用純 `npm run dev`
在本機測試**，跟聊天助手那支 `/api/chat.js` 一樣，只能用 `vercel dev` 或直接部署到 Vercel
上測試。

**這是社群協作的地圖資料**，完整度依地區而異：東京、橫濱這種市區通常查得到不少店家，
但像成田機場周邊、北輕井澤山區度假村這種偏遠地點，可能真的查不到什麼——畫面顯示「無」
或很少結果時，這是真實資料的樣貌，不是程式漏抓。

因為 Overpass 是大家共用的免費公共伺服器，查過的地點結果會存 localStorage，**7 天內同一個
地點不會重複查詢**，這是為了不要對這個公共資源造成太多負擔，也讓 App 反應更快，請不要縮短
這個快取時間。
